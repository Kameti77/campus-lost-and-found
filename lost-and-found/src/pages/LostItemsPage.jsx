...
function LostItemsPage({ onCardClick, onItemsLoaded }) {
  const { filteredItems, loading, error, fetchItems, items } = useFilteredItems("lost");
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {loading && (
        <div className="flex flex-col items-center py-20">
          <div className="h-10 w-10 border-4 border-red-400 border-t-transparent rounded-full animate-spin" />
          <p className="mt-4 text-gray-500">Loading lost items...</p>
        </div>
      )}
      ...
      {!loading && !error && filteredItems.length === 0 && (
        <div className="bg-white border rounded-xl p-10 text-center shadow-sm">
          ...
          <h3 className="text-lg font-semibold text-gray-800">No lost items reported</h3>
